...
class DquEmptyCheck(BaseDQCheck):
    """
    Empty/Blank Value Check Rule:
    Identifies rows with null or blank values in specified columns.
    ...
    def run(self, evaluation="basic"):
        ...
        return runner.run_empty_check(columns, evaluation=evaluation)
    
    @classmethod
    def expected_config(cls):
        return {
            "columns": (list, True)  # Required: List of column names to check for empty/null values
        }